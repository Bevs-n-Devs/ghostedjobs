...
func SearchRating(w http.ResponseWriter, r *http.Request) {
	...
	reviewsByRating, err := db.GetAllReviewsByReviewRating(reviewRating)
	if err != nil {
	...
	for index := range reviewsByRating {
		var convertData ViewGhostedReviews

		convertData.InteractionType = reviewsByRating[index].InteractionType
		convertData.ReviewRating = reviewsByRating[index].ReviewRating
		convertData.CreatedAt = reviewsByRating[index].CreatedAt

		// decrypt data
		companyName, err := utils.Decrypt(reviewsByRating[index].CompanyName)
		if err != nil {
		...
		recruiterName, err := utils.Decrypt(reviewsByRating[index].RecruiterName)
		if err != nil {
		...
		managerName, err := utils.Decrypt(reviewsByRating[index].ManagerName)
		if err != nil {
		...
		reviewContent, err := utils.Decrypt(reviewsByRating[index].ReviewContent)
		if err != nil {
		...
		profileName, err := utils.Decrypt(reviewsByRating[index].ProfileName)
		if err != nil {
